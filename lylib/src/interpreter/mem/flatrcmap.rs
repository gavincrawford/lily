//! A generic flat map backed by a `Vec`, indexed by interned `usize` identifiers.
...
impl<T: PartialEq> FlatRcMap<T> {
    ...
    pub(crate) fn get(&self, id: usize) -> Option<Rc<RefCell<T>>> {
        self.map.get(id).and_then(|value| value.clone())
    }

    /// Inserts a value into the map, returning the previous value if it existed.
    pub(crate) fn insert(&mut self, id: usize, value: Rc<RefCell<T>>) -> Option<Rc<RefCell<T>>> {
        if id < self.map.len() {
